...
export default function PlacementRequestForm() {
  ...
  const handleSubmit = (e) => {
    ...
    const token = localStorage.getItem("token");
    if (!token) {
      setResponseMessage(<Alert variant="Danger">You are not authenticated.</Alert>);
      return;
    }

    if (!organization || !profile || !description) {
      setResponseMessage(<Alert variant="Danger">Please fill out all required fields</Alert>);
      return;
    }
    ...
    axios
      ...
      .catch((error) => {
        console.error(<Alert variant="Danger">Error submitting placement request:</Alert>, error);
        setResponseMessage(<Alert variant="Danger">Failed to submit placement request.</Alert>);
      });
  };
